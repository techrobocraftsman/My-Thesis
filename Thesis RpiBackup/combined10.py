...
#****Draw contours
    ...
    for cnt in contours:
        ...
        if area > 500:
            ...
            change = True
    global static,state,pos,flag,done
    inc = 1
    ...
    wait = 20
    wait2= 50
    originX, originY = (frameW//2)-70, frameH-70
    if static<wait :
        if change:
        ...
        if change:
            ...
            if abs(dx)<=tolerance and abs(dy)<=tolerance+5:
                static+=1
                print(static)
            else:static = wait
        ...
        if static==wait:Msg = f'{mDir}{speed} {tDir}{turn} y{ry} U{az} r{offset}'
...
ry,az,offset,grip,rz = 0,65,50,'r',85
...
while True:    
    ...
    if state==1:
        ry,az,offset,grip,rz = 130,100,0,'r',0
        initMsg = f'F{0} R{0} y{ry} U{az} r{offset}'
        comm.sendData(initMsg)
        comm.readData()
        state = 10
